...
{
    public class GetUserAccessCommand : ObjectCommand
    {
        private readonly SqlParameter _returnValue;

        public GetUserAccessCommand(ObjectConnection objectConnection)
            : this(objectConnection.CreateCommand())
        {
        }

        public GetUserAccessCommand(SqlCommand sqlCommand)
            : base(sqlCommand)
        {
            Command.CommandText = "[dbo].[spGettblUserAccess]";
